...
const db = new sqlite3.Database('ferreteria.db');
...
app.post('/ventas', (req,res)=>{
    ...
    ids.forEach((id,i)=>{
        db.get("SELECT * FROM productos WHERE id = ?", [id], (err,p)=>{
            ...
            db.run("UPDATE productos SET stock = stock - ? WHERE id = ?", [cantidad, id]);
...
app.get('/editar/:id',(req,res)=>{
    db.get("SELECT * FROM productos WHERE id=?",[req.params.id],(e,p)=>{
        res.send(`
        <html><head>${estilo}</head><body>
        <div class="container">
        <div class="topbar">
